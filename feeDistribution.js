...
const connection = new Connection(
  "https://mainnet.helius-rpc.com/?api-key=ff3c6e4e-511d-4b13-9ed7-7beab8f250e5",
  "confirmed"
);
...
async function distributeFees() {
  ...
  const { accountsToWithdrawFrom, amounttowithdraw } =
    await accountstowithdrawFee(allacounts);
  if (accountsToWithdrawFrom && accountsToWithdrawFrom.length > 0) {
    ...
    console.log(
      ...
      "Withdraw transfer fee to sender account."
    );
  }
  ...
  await sendSPLTokensToAddress(
    sender,
    addressuser,
    reflectionsToSpecificWallet
  );
}
async function sendSPLTokensToAddress(sender, receiverPublickKey, amount) {
  ...
  if (ins) {
    let transaction = new Transaction().add(ins);
  ...
  console.log(
    transactionSignature,
    `Transfered ${parseInt(transferAmount) / 10 ** 4} to wallet`
  );
}
async function sendSPLTokensToPool(sender, amount) {
  ...
  console.log(
    transactionSignature,
    `Transfered ${parseInt(amount) / 10 ** 4} to pool`
  );
}
async function distributeReflections(sender, amount, allAccounts) {
  ...
  for (const account of accounts) {
    ...
    console.log(
      transactionSignature,
      `Transfered ${parseInt(transferAmount) / 10 ** 4} to token holder`
    );
  }
}
...
async function distributeNFTReflections(sender, amount) {
  ...
  for (const account of allHolders) {
    const [ins, receiver] = await createTokenAccount(
      mint,
      new PublicKey(account.owner)
    );
    if (ins) {
      transaction = new Transaction().add(ins);
      let transaction = new Transaction().add(createAccountInstruction);
      transactionSignature = await sendAndConfirmTransaction(
        connection,
        transaction,
        [payer]
    ...
    console.log(
      transactionSignature,
      `Transfered ${parseInt(transferAmount) / 10 ** 4} to NFT holder`
    );
  }
}
...
const getNFTsByCreator = async () => {
  const res = await umi.rpc.getAssetsByCreator({
    creator: new PublicKey("2tURYHc9JqntFh14kRuoJNrwbenQSJ5fErAuDUUzpudr"),
    onlyVerified: true,
    page: 1,
    limit: 1000,
  });
  var holders = res.items.map((item) => {
    return { owner: item.ownership.owner };
  });
  return holders;
};
